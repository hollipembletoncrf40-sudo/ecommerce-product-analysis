...
1. 在报告开篇，请根据类目和描述，**首先推荐 3 个具体的可行产品方向**，并简述理由。
2. 然后，**选择其中最具潜力的一个产品方向**，以此作为本次分析的主体，完成后续的15维深度商业分析。
3. 请在报告中明确说明你选择的是哪个产品方向进行分析的。

===== 输出质量强制要求 =====

**【严格遵守】每个维度的内容深度要求**：
1. **每个一级维度（如"产品定位与核心价值"）必须包含至少 500-800 字的详细分析**，不得简略带过。
2. **每个子维度（如"功能属性"、"情感属性"）必须展开 3-5 个具体要点**，每个要点需包含：
   - What（是什么）：明确定义
   - Why（为什么重要）：逻辑推导
   - How（如何落地）：具体执行建议
3. **凡涉及数据（价格、成本、市场规模等）必须提供合理估算区间和估算依据**。
4. **凡涉及策略建议必须提供 2-3 个可选方案并对比优劣**。
5. **禁止出现"等"、"诸如此类"等模糊表述**，必须穷尽列举或明确范围。
6. **每段文字不得少于 3 行**，严禁一句话带过。

请务必按照以上深度要求输出内容，**内容不够详细将被视为不合格**。

请结合以上信息，按照分析框架输出一份**15000字以上的万字级深度**专业报告。这是一份付费报告，质量必须对得起用户投资的时间和金钱。` }
      ],
